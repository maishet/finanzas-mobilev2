...
export default function MovementsScreen() {
  ...
  const movementsQuery = useQuery({ queryKey: ['transactions'], queryFn: () => apiRequest<Transaction[]>('/api/transactions'), retry: false })
  ...
  return (
    <Screen isRefreshing={movementsQuery.isRefetching} onRefresh={() => movementsQuery.refetch()}>
      <XStack items="center" justify="space-between" gap="$3">
        <H2 size="$7" color="$color12" flex={1}>{t('tabs.movements')}</H2>
        <Link href="/transaction-form" asChild><FintButton size="$3">{t('actions.newMovement')}</FintButton></Link>
      </XStack>
      {movements.length === 0 ? <DataStateCard message={t('empty.movements')} /> : movements.map((movement) => (
